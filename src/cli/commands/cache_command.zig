...
pub fn run(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    runWithCli(allocator, cli) catch |err| {
        if (cli.json_output) {
            try cli_output.printSimpleFailureJson(allocator, "cache", @errorName(err));
        } else {
            cli_output.printSimpleFailureHuman("cache", @errorName(err));
        }
        return err;
    };
}

fn runWithCli(allocator: std.mem.Allocator, cli: cli_types.CacheCliArgs) !void {
